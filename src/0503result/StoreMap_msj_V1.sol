...
contract StoreMap{
	...
	{	
		...
		bytes32 gtype;
		uint256 path_num;
		uint256 vHistory;	//--msj:History path situation
		uint256 vCurrent;	//--msj:Current path situation
		uint256 vFact;		//--msj:combine History path situation and Current path situation,the Fact path situation
		uint256 vDefault;	//--msj:the Default path situation
    ...
    //now Vehicle
	ToCalVehicle nowVehicle;

    function getRoutesFromTraffic(bytes32 vehicleId, bytes32[] memory route, uint256 cost) public {
        nowVehicle.VehicleId = vehicleId;
		nowVehicle.routes[vehicleId].routeCoords = route;
		nowVehicle.routes[vehicleId].routeCost = cost;
    }
	...
	// cal the vehicle speed
	function calVehicleSpeed() public returns(uint256){
		// nowVehicle.vehicleSpeed = nowVehicle.routes[nowVehicle.VehicleId].routeCost / runtime;
		// uint256 vehicleSpeed = 40;
		// runtime = 5
		if(astarCount >= 1 && astarCount < 50){
			nowVehicle.vehicleSpeed = 70;
		}
		else if(astarCount >= 50 && astarCount < 150){
			nowVehicle.vehicleSpeed = 40;
		}
		else if(astarCount >= 150 && astarCount <200){
			nowVehicle.vehicleSpeed = 100;
		}
		else{
			astarCount=1;
			nowVehicle.vehicleSpeed = 80;
		}
		return nowVehicle.vehicleSpeed;
	}

	// cal the path current situation
	function calvCurrent(uint256 gid) public {
		if(types[gid].vCurrent==0){
			//types[gid].vHistory = types[gid].vCurrent;
			types[gid].vCurrent = calVehicleSpeed();
		}
		else{
			types[gid].vCurrent = (types[gid].vCurrent + calVehicleSpeed()) / 2;
	...
	// cal the path fact situation
	function calTraffic(bytes32 sourceGeo) public {
		uint256 a = 8;
		uint256 thegid;
		thegid = geotogid.sGtG[sourceGeo];
		// cal path situation
		calvCurrent(thegid);
		types[thegid].vFact = (types[thegid].vHistory*a+types[thegid].vCurrent*(10-a))/10;
		types[thegid].vHistory = types[thegid].vCurrent;
		astarCount++;
	}

	// cal the addtional path cost
    function calPathCostToAdd(uint256 gid) public view returns(uint256){
		uint256 b = 5;
        //pathCostToAdd = routes[vehicleId].routeCost*b;
		// calTraffic(gid);
		uint256 pathCostToAdd = (types[gid].vDefault - types[gid].vFact)*b;
		return pathCostToAdd;
		//return 50;
    }

	// test code
	uint256 testTime_astar = 0;
	function testBug_astar() public returns(uint256){
		testTime_astar++;
		return testTime_astar;
	}
	...
	// 添加一条
	// be called in uploadmap_cjz_3.js
	function add_onetype(uint256 gid, int32 minzoom, int32 cost, int32 source, int32 target, bool oneway, bool building, bytes32 highway, bytes32 name, bytes32 gtype, bytes32[] memory path) public {
		...
		types[gid].vCurrent = 80;	//--msj:default speed is 80km/h
		types[gid].vHistory = 0;	//--msj:default speed is 0km/h
		types[gid].vFact = 80;		//--msj:default speed is 80km/h
		types[gid].vDefault = 80;	//--msj:default speed is 80km/h, it is a constant value
		...
		geotogid.sGtG[path[0]] = gid;
		geotogid.tGtG[path[path.length-1]] = gid;
	}
	...
	function astar(bytes32 startGeohash, bytes32 endGeohash) public returns(bytes32[] memory backwards, uint256 costAll){
		// enqueue(bytes32 geohash, uint256 cost)
		// testastar = testBug_astar();	// time 1
		...
		while (frontier.geohashs.length > 1) {
			// testastar = testBug_astar();		// time 2
			currentGeohash = top(); 
			...
			for (uint256 i = 0; i < adjNodes.adjnum; i++) {
				// testastar = testBug_astar();		// time 3
				...
				if (costSofar.map[adjNodes.adjs[i].targetGeohash] == 0 || newCost < costSofar.map[adjNodes.adjs[i].targetGeohash]) {
					// testastar = testBug_astar();		// time 4
					...
					// uint256 gid = geo_maps[hash].types_list[i];
					uint256 gid = 0;		// to find gid!!!
					uint256 thenum = geo_maps[currentGeohash].num;
					gid = geo_maps[currentGeohash].types_list[thenum];
					priority = newCost * P + manhattan(adjNodes.adjs[i].targetGeohash, endGeohash) + calPathCostToAdd(gid);
					// astarCount++;
					// ---msj---add there
