...
contract StoreMap{
	...
	{	
		...
		uint256 vHistory;	//--msj
		uint256 vFact;		//--msj
		uint256 vCurrent;	//--msj
		uint256 vDefault;	//--msj
	}
	...
	//-----------------msj--------------------------------------------------
	// geohash -> gid, be used in add_line
	// struct GeoToGid{
	//	mapping (bytes32 => uint256) sGtG;
	//	mapping (bytes32 => uint256) tGtG;
	// }
	// GeoToGid geotogid;
	mapping (bytes32 => uint256) sGtG;

	// get the A* result, in order to update path speed, be called in web as ".send()"  
	// change state in blockchain
	function updatePathSituation(bytes32[] memory astarOriginRoute) public {
		for(uint256 i=0;i<astarOriginRoute.length;i++){
			if(astarOriginRoute[i] != 0){
				uint256 gid = sGtG[astarOriginRoute[i]]; 
				types[gid].vCurrent = calPathSpeed(calVehicleSpeed());
				if(types[gid].vCurrent > types[gid].vDefault){
					types[gid].vCurrent = types[gid].vDefault;
				}
				calPathSituation(gid);
				types[gid].vHistory = types[gid].vFact;
			}
		}
		aCount++;
	}
	...
	// calculate the vehicle speed, be used in updatePathSituation()
	// not change state in blockchain
	function calVehicleSpeed() view public returns(uint256){
		uint256 vehicleSpeed;
		if(aCount == 1){
			vehicleSpeed = 80;
		}
		else{
	...
	// calculate the path speed, be used in updatePathSituation()
	// not change state in blockchain
	function calPathSpeed(uint256 vSpeed) pure public returns(uint256){
		uint256 pSpeed = vSpeed;
		return pSpeed;
	}

	// calculate the path situation, be used in updatePathSituation()
	// change state in blockchain
	function calPathSituation(uint256 gid) public {
		uint256 aef = 8;
		types[gid].vFact = (aef*types[gid].vHistory + (10-aef)*types[gid].vCurrent)/10;
	} 

	// calculate the path cost to be added, be used in astar()
	// not change state in blockchain
	function calPathCostToAdd(uint256 gid) view public returns(uint256){
		uint256 b = 15;
		uint256 cost = (types[gid].vDefault - types[gid].vFact)*b;
		return cost;
	}

	// to test the geotogid if it work successfully
	// be called in web as ".call"
	function printgeotogid(bytes32[] memory astarOriginRoute) view public returns (uint256 len, uint256 gids){
		len = astarOriginRoute.length;
		// for(uint256 i=0;i<astarOriginRoute.length;i++){
		// 	if(astarOriginRoute[i] != 0){
		// 		gids = sGtG[astarOriginRoute[i]];
		// 	}
		// }
		gids = sGtG[astarOriginRoute[len-1]];
	}

	// to test the types if it can be bind with speed
	// be called in web as ".call"
	function printfactspeed(bytes32[] memory astarOriginRoute) view public returns (uint256 vh,uint256 vc,uint256 vf,uint256 vd){
		for(uint256 i=0;i < astarOriginRoute.length;i++){
			if(astarOriginRoute[i] != 0){
				vh = types[sGtG[astarOriginRoute[i]]].vHistory;
				vc = types[sGtG[astarOriginRoute[i]]].vCurrent;
				vf = types[sGtG[astarOriginRoute[i]]].vFact;
				vd = types[sGtG[astarOriginRoute[i]]].vDefault;
			}
		}
	}

	// to test whether the sGtG has been updated successfully
	// be called in web as ".call" 
	function testsGtG(bytes32 testgeo) view public returns(uint256 testgid){
		testgid = sGtG[testgeo];
	}
	//-----------------msj--------------------------------------------------
	...
	function add_onetype(uint256 gid, int32 minzoom, int32 cost, int32 source, int32 target, bool oneway, bool building, bytes32 highway, bytes32 name, bytes32 gtype, bytes32[] memory path) public {
		...
		types[gid].vHistory = 80;		// --msj
		types[gid].vFact = 80;			// --msj
		types[gid].vCurrent = 80;		// --msj
		types[gid].vDefault = 80;		// --msj
		...
		sGtG[path[0]] = gid;					// --msj
		//testsGtG = sGtG[path[0]];
		// geotogid.tGtG[path[path.length-1]] = gid;		// --msj
	...
	//astar算法主流程逻辑
	function astar(bytes32 startGeohash, bytes32 endGeohash) public returns(bytes32[] memory backwards, uint256 costAll, uint256 gasCost,uint256 times){
		uint256 u0 = gasleft();
		...
		while (frontier.geohashs.length > 1) {
			...
			if (currentGeohash == endGeohash) {
				...
				uint256 u1 = gasleft();
				gasCost = u0 - u1;			// 使用的gas值，用于判断时间or空间复杂度
				break;
			}
			...
			for (uint256 i = 0; i < adjNodes.adjnum; i++) {
				...
				uint256 newCost = costSofar.map[currentGeohash] + uint256(adjacencyList[currentGeohash].adjs[i].cost) + calPathCostToAdd(sGtG[currentGeohash]);
				...
				// --msj
					...
					// uint256 thegid = 0;
					// uint256 thenum = geo_maps[currentGeohash].num;
					// thegid = geo_maps[currentGeohash].types_list[thenum];
					priority = newCost * P + manhattan(adjNodes.adjs[i].targetGeohash, endGeohash) + calPathCostToAdd(sGtG[adjNodes.adjs[i].targetGeohash]);
					// psCost = calPathCostToAdd(sGtG[adjNodes.adjs[i].targetGeohash]);
					// psCost = calPathCostToAdd(thegid);
					uint256 psCost = 1;
					// 保存一下最大的psCost试试看
					if(psCost <= calPathCostToAdd(sGtG[adjNodes.adjs[i].targetGeohash])){
						psCost = calPathCostToAdd(sGtG[adjNodes.adjs[i].targetGeohash]);
					}
					// --msj
		...
		times = aCount;		// to output the aCount
	}
