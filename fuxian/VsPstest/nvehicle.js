...
//Map contract
let mapContractAddress = '0xeca420f01f2d1dc64237aa9a87dc04de68afa15b';
let mapContractAbi = JSON.parse(fs.readFileSync('./mapAbi.json', 'utf-8'));
let mapContract = new web3.eth.Contract(mapContractAbi,mapContractAddress);

//Traffic contract
let trafficContractAddress = '0x3e70f4d2c7e52c26034834fb820ee8acf05523ea';
...
let vehiclePositionList =["wx4ernfyb2x"];
...
fs.writeFileSync("./vehicleTimeInfo.json","[",{flag:'w',encoding:'utf-8',mode:'0666'},function(err){});
initVehicle();
// deleteVehicle();
// getVehicleIdList();
	...
	trafficContract.events.payEvent(function(error, event){
		...
		if(event.returnValues.vehicleId.slice(0,42) == vehicleId.toLowerCase()){
            ...
            trafficContract.methods.initVehicle(vehicleId, web3.utils.asciiToHex(vehicleMessage.endGeohash)).send({from: vehicleId, gas: 500000,position: vehicleMessage.endGeohash, txtime:Date.now()}).then(function(result){
				...
				fs.writeFileSync("./vehicleTimeInfo.json",JSON.stringify(timeInfo),{flag:'a',encoding:'utf-8',mode:'0666'},function(err){});
                ...
                if(countNum == systemNum){
					...
					// fs.writeFileSync("./timeInfo/vehicleTimeInfo.json",JSON.stringify(allTimeInfo),{flag:'w',encoding:'utf-8',mode:'0666'},function(err){});
					 console.log("finish");
					 fs.writeFileSync("./vehicleTimeInfo.json","]",{flag:'a',encoding:'utf-8',mode:'0666'},function(err){});
				}else{
					fs.writeFileSync("./vehicleTimeInfo.json",",",{flag:'a',encoding:'utf-8',mode:'0666'},function(err){});
				}
            })
